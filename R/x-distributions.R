...
x_distribution_app <- function() {
  ui <- page_sidebar(
    ...
    sidebar = sidebar(
      radioButtons(
        ...
        choices = c("Normal", "Skewed", "Bimodal"),
        selected = "Normal"
      ),
      ...
      radioButtons(
        "residual_plot",
        "Residual plot",
        choices = c("None", "Histogram", "QQplot"),
        selected = "None"
      )
    ),
  ...
  horiz_hist <- function(
    ...
    hbreaks <- a$breaks
    hpos <- function(pos) {
      (pos - hbreaks[1]) * (length(hbreaks) - 1) / diff(range(hbreaks))
    }
    if (plot) {
      barplot(
        if (freq) a$counts else a$density,
        space = 0,
        horiz = TRUE,
        ylim = hpos(ylim),
        col = col,
  ...
  server <- function(input, output, session) {
    ...
    output$distPlot <- renderPlot({
      layout(
        mat = matrix(c(1, 2, 4, 0, 3, 0), nrow = 3, ncol = 2),
      ...
      if (input$residual_plot == "Histogram") {
        ...
        lines(
          x_norm,
          dnorm(x_norm, mean = 0, sd = sd(resid(mod()))),
          col = "#832424",
          xlab = "Residuals",
