...
coef_explain_app <- function() {
  ui <- page_sidebar(
    sidebar = sidebar(
      checkboxGroupInput("pred", "Predictor",
        choiceValues = c("Food", "Temperature", "Interaction"),
        choiceNames = c(
          "Food (A vs. B)",
  ...
  server <- function(input, output, session) {
    data <- reactive({
      ...
      temperature <- rep(c("Low", "High"), each = 24) |>
        factor(levels = c("Low", "High"))
      response <- b0 + b1 * (food == "B") + b2 * (temperature == "High") +
        b3 * (food == "B" & temperature == "High") + rnorm(48)
    ...
    output$plot <- renderPlot({
      ...
      arrows(xs, rep(0, length(pred)), xs, rep(coefs()[1], length(pred)),
        col = coef_colours()[1], lwd = 2, length = 0.1
      )
      ...
      if ("foodB" %in% names(coefs())) {
        arrows(pos, coefs()[1], pos, coefs()[1] + coefs()["foodB"],
          col = coef_colours()["foodB"], lwd = 2, length = 0.1
        )
      ...
      if ("temperatureHigh" %in% names(coefs())) {
        arrows(pos, coefs()[1], pos, coefs()[1] + coefs()["temperatureHigh"],
          col = coef_colours()["temperatureHigh"], lwd = 2, length = 0.1
        )
        pos <- pos + 1
      }
      ...
      if (all(c("foodB", "temperatureHigh") %in% names(coefs()))) {
        ...
        arrows(pos,
          ...
          col = coef_colours()["temperatureHigh"],
          lwd = 2, length = 0.1
        )
