    ...
    sidebar = sidebar(
      accordion(
        accordion_panel(
          ...
          radioButtons("dist", "Distribution", choices = c("Poisson", "Negative Binomial")),
          sliderInput("mean",
                      "Mean",
                      min = 0,
                      max = 10,
        ...
        accordion_panel(
          ...
          sliderInput("zero", "Proportion excess zeros", min = 0, max = 1, value = 0),
  ...
  server <- function(input, output) {
    output$negbin <- renderUI({
      if (input$dist == "Negative Binomial") {
        ...
        list(
          sliderInput("variance", "Variance", min = input$mean, max = 20, value = 10),
          p("With the negative binomial distribution, the variance can change independently of the mean, giving the distribution more flexibility.")
        )
      } else {
    ...
    output$distPlot <- renderPlot({
      ...
      segments(0, y[x==0], 0, y[x==0] + (input$zero), col = "#832424", lwd = 10, lend = 1)
    })
  }
  # Run the application
