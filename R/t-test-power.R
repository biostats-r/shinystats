...
t_test_power_app <- function() {
  ...
  ui <- page_sidebar(
    ...
    sidebar = sidebar(
      ...
      radioButtons("alternative", "Alternative", choices = c("Two-sided", "One-sided - greater than", "One-sided - less than"), selected = 0.05),
  ...
  server <- function(input, output, session) {
    output$distPlot <- renderPlot({
      ...
      alpha <- switch(input$alternative,
        ...
        "One-sided - greater than" = alpha,
        "One-sided - less than" = 1 - alpha
      )
